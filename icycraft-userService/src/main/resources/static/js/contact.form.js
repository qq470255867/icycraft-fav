$(function() {
	...
	$('.contact-form').on("submit", function(){
		...
		if ($.trim($('.contact-form input[name="userName"]').val()) === '') {error = 1; $this.find('input[name="userName"]').parent().addClass('invalid'); msg = msg +  '\n - 用户名';}
		if ($.trim($('.contact-form input[name="passWord"]').val()) === '') {error = 1; $this.find('input[name="passWord"]').parent().addClass('invalid'); msg = msg +  '\n - 密码';}
        if (error){
